...
- без backend и без реального аккаунта на текущем этапе.
...
- доработать multi-user ready слой без подключения backend;
- описать серверные контракты и будущую sync-модель;
- только после этого добавлять backend skeleton и account mode.
